...
type fakeCoreServerForTelemetry struct {
	traffic []domain.UserTraffic
}

func (f *fakeCoreServerForTelemetry) AddNode(_ string, _ *domain.NodeInfo) error { return nil }
...
func (f *fakeCoreServerForTelemetry) DelUsers(_ []domain.UserInfo, _ string, _ *domain.NodeInfo) error {
	return nil
}
func (f *fakeCoreServerForTelemetry) GetUserTrafficSlice(_ string, _ int) ([]domain.UserTraffic, error) {
...
func TestReportUserTrafficTask_OnlineUserFilterPreserved(t *testing.T) {
	...
	limiter.Init()
	lim := limiter.AddLimiter(c.tag, []domain.UserInfo{
		{Id: 1, Uuid: "uuid-1"},
		{Id: 2, Uuid: "uuid-2"},
	}, nil)
